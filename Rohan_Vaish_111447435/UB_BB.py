...
def classify_all_models(training_data, train_data, testing_data, test_data,
                        gram, table_entries, LC_dict, step_size):
    Label = ''
    if gram is 'unigram':
        Label = 'UB'
        ...
        table_entries.append([
            ...
            float(
                metrics.precision_score(
                    test_data.target, pred_class, average='macro')),
            float(
                metrics.recall_score(
                    test_data.target, pred_class, average='macro')),
            float(
                metrics.f1_score(test_data.target, pred_class, average='macro'))
        ])
        LC_dict.setdefault(algo_name, {'Size': [], 'F1': []})
        LC_dict[algo_name]['Size'].append(step_size)
        LC_dict[algo_name]['F1'].append(
            float(
                metrics.f1_score(test_data.target, pred_class,
                                 average='macro')))
        i += 1
...
if __name__ == '__main__':
    ...
    uni_train_data, uni_test_data, bi_train_data, bi_test_data = tokenizer(
    ...
    classify_all_models(training_data, train_data, testing_data, test_data,
                        'unigram', table_entries, LC_dict,
                        uni_train_data.shape[0])

    training_data = bi_train_data
    testing_data = bi_test_data
    classify_all_models(training_data, train_data, testing_data, test_data,
                        'bigrams', table_entries, LC_dict,
                        bi_train_data.shape[0])
    ...
    while step_size <= train_size:
        classify_all_models(training_data, train_data, testing_data, test_data,
                            'unigram', table_entries, LC_dict, step_size)
        step_size += 100

    if step_size > train_size and train_size % step_size is not 0:
        classify_all_models(training_data, train_data, testing_data, test_data,
                            'unigram', table_entries, LC_dict, train_size)
